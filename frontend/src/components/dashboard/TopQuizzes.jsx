...
const TopQuizzes = ({ quizzes, darkMode }) => {
  const sortedQuizzes = [...quizzes]
    .sort((a, b) => (b.stats?.plays || 0) - (a.stats?.plays || 0))
    .slice(0, 5);

  return (
    <div className={`rounded-2xl p-6 h-full transition-colors ${
      darkMode 
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Топ викторин</h3>
        </div>
        <Link 
          to="/my-quizzes"
      ...
      {sortedQuizzes.length > 0 ? (
          ...
          {sortedQuizzes.map((quiz, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`font-medium truncate group-hover:text-purple-500 transition-colors ${
        ...
        <div className="text-center py-8">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3 ${
            ...
            <Brain className={`w-7 h-7 ${darkMode ? 'text-gray-400' : 'text-gray-400'}`} />
          </div>
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Создайте первую викторину</p>
          <Link 
            to="/create-quiz"
