...
const Sidebar = ({ darkMode, collapsed, onToggle }) => {
  ...
  const mainNavItems = [
    ...
    { icon: Book, label: "Словарь", path: "/vocabulary", badge: "Новое" },
              ...
              <div className="text-left">
                <p
                  className={`text-lg font-bold ${
                    darkMode ? "text-white" : "text-gray-900"
                  } ${animateXP ? "text-purple-400" : ""} transition-colors`}
                >
            ...
            <div className="flex justify-between text-[10px] mt-1">
              ...
              <span className={darkMode ? "text-gray-500" : "text-gray-400"}>
                {levelInfo.nextLevel !== Infinity
                  ? `${(
                      levelInfo.nextLevel - totalPoints
                    ).toLocaleString()} XP до след.`
                  : "🎉 MAX"}
              </span>
            </div>
          </div>
          ...
          >
            ...
            <div
              className={`grid grid-cols-3 divide-x ${
                darkMode
                  ? "divide-white/10 bg-white/[0.02]"
                  : "divide-gray-100 bg-gray-50"
              }`}
            >
